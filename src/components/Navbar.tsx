...
const Navbar = () => {
  ...
  return (
    <section className="w-full bg-white/70 h-20 shadow-md">
        ...
        <div className="hidden md:inline-flex items-center gap-7 text-gray-900 hover:text-black duration-200">
          {navigation.map((item) => (
            ...
            >
              {item?.title}
              <span
                className="w-full h-[1px] bg-black absolute inline-block left-0 bottom-0 
                -translate-x-[100%] group-hover:translate-x-0 transition-transform duration-200"
              />
            </Link>
          ))}
        </div>
